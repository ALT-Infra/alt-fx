...
function startCliUpgradeServer(
  root: string,
  options: {
...
describe("cli: status", () => {
  ...
  test(
    "fx upgrade uses the controlled loopback CDN for the epoch manifest and artifacts",
    async () => {
      ...
      const release = startCliUpgradeServer(root, {
  ...
  test(
    "fx upgrade falls back to epoch zero and refuses a legacy downgrade",
    async () => {
