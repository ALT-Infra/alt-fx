...
pub const top_level_help_groups = [_]TopLevelHelpGroup{
    ...
    .{ .entries = &.{
        .{ .kind = .login, .usage = "login [vercel|codex|grok|opencode|parallel]", .summary = "Sign in to a model provider" },
        .{ .kind = .logout, .usage = "logout [vercel|codex|grok|opencode|parallel]", .summary = "Sign out of a model provider" },
        .{ .kind = .provider, .usage = "provider <gateway|codex|grok|opencode>", .summary = "Choose the active model provider" },
