...
test "runtime context composes exact auto mode with noninteractive blockers and sandbox" {
    ...
    try std.testing.expectEqualStrings(
        "Runtime context: permission mode is auto. After configured rules, session grants, and deterministic direct-command authority, Fx reviews each unresolved sensitive tool call once. The reviewer either allows that exact action or sends it to the user for approval. Tool admission remains authoritative.",
