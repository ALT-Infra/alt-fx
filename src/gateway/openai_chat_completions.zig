...
pub const ResolveRouteFn = *const fn ([]const u8) Route;
pub const ModelPredicateFn = *const fn ([]const u8) bool;
...
pub fn provider(spec: *const Spec) stream_provider.Provider {
    return .{
        .context = @ptrCast(@constCast(spec)),
        .stream_fn = streamCompletion,
    };
}

fn specFromContext(raw: ?*anyopaque) *const Spec {
    return @ptrCast(@alignCast(raw.?));
}

fn validCredential(value: []const u8) bool {
    if (value.len == 0 or value.len > 8 * 1024) return false;
    for (value) |byte| if (byte < 0x20 or byte == 0x7f) return false;
    return true;
}

fn validateModel(model: []const u8, spec: *const Spec) !void {
    if (model.len == 0 or model.len > 256) return error.InvalidOpenAICompatibleModel;
    for (model) |byte| {
        if (byte <= 0x20 or byte == 0x7f) return error.InvalidOpenAICompatibleModel;
    }
    const wire_model = spec.resolve_route(model).wire_model;
    if (wire_model.len == 0) return error.InvalidOpenAICompatibleModel;
}
...
pub fn buildRequest(
    ...
    const wire_model = spec.resolve_route(request.model).wire_model;
    ...
    if (request.max_output_tokens) |limit| {
        const completion_tokens = if (spec.uses_max_completion_tokens) |predicate|
            predicate(wire_model)
        else
            false;
...
fn writeMessages(
    ...
    for (messages, 0..) |message, index| {
        ...
        switch (message.role) {
            ...
            .assistant => {
                ...
                if (message.tool_calls.len > 0) {
                    ...
                    for (message.tool_calls) |call| {
                        ...
                        {
                            return error.InvalidOpenAICompatibleToolCall;
                        }
                        if (tool_first) {
                            tool_first = false;
                        } else {
            ...
            .tool => {
                const tool_call_id = message.tool_call_id orelse return error.InvalidOpenAICompatibleToolCall;
...
fn streamCompletion(
    ...
    if (request.credential.source != spec.credential_source or
        !validCredential(request.credential.secret))
    {
        return error.OpenAICompatibleCredentialRequired;
    }
    try validateModel(request.model, spec);
    const payload = try buildRequest(alloc, request.data(), spec);
    defer alloc.free(payload);

    var result = streamCompletionCore(alloc, request, payload, spec) catch |err| {
        if (request.cancel_flag.load(.seq_cst)) return error.Cancelled;
...
fn streamCompletionCore(
    ...
    if (response.head.status != .ok) {
        ...
        const bounded_body = reader.allocRemaining(alloc, .limited(max_error_body_bytes + 1)) catch |err| switch (err) {
            error.StreamTooLong => try std.fmt.allocPrint(alloc, "{s} error response exceeded the local limit", .{spec.provider_name}),
            else => return err,
        };
        const body = if (bounded_body.len > max_error_body_bytes) body: {
            alloc.free(bounded_body);
            break :body try std.fmt.allocPrint(alloc, "{s} error response exceeded the local limit", .{spec.provider_name});
        } else bounded_body;
        return .{ .failed = .{
            .kind = failureKindWithDetail(response.head.status, body, spec),
            .detail = body,
            .ownership = .owned,
        } };
    }
...
fn failureKindWithDetail(
    status: std.http.Status,
    detail: []const u8,
    spec: *const Spec,
) stream_provider.FailureKind {
    if (status == .too_many_requests and isNonRetryableLimitError(detail, spec)) return .provider_error;
    return failureKind(status);
}

fn isNonRetryableLimitError(detail: []const u8, spec: *const Spec) bool {
    for (spec.non_retryable_limit_markers) |marker| {
        if (std.mem.find(u8, detail, marker) != null) return true;
    }
    return false;
}
...
fn consumeSse(
    ...
    while (try sse.next(alloc, reader)) |json_text| {
        ...
        var parsed = std.json.parseFromSlice(std.json.Value, alloc, json_text, .{}) catch |err| switch (err) {
            ...
            else => return error.InvalidOpenAICompatibleSseEvent,
        };
        defer parsed.deinit();
                    ...
                    if (tool_calls_value == .array) {
                        for (tool_calls_value.array.items) |call_value| {
                            ...
                            if (accumulator_index == null) {
                                if (tools.items.len >= max_tool_calls) return error.OpenAICompatibleResourceLimitExceeded;
    ...
    // closed early.
    if (!sse.saw_done or finish_reason == null) return error.OpenAICompatibleStreamIncomplete;
    ...
    for (tools.items, 0..) |*tool, index| {
        if (tool.id.len == 0 or tool.name.len == 0) return error.InvalidOpenAICompatibleSseEvent;
...
fn checkedAccumulatedSize(current: usize, incoming: usize, limit: usize) !usize {
    const total = std.math.add(usize, current, incoming) catch return error.OpenAICompatibleResourceLimitExceeded;
    if (total > limit) return error.OpenAICompatibleResourceLimitExceeded;
    return total;
}
...
const test_endpoint_zen = "https://opencode.ai/zen/v1/chat/completions";
const test_endpoint_go = "https://opencode.ai/zen/go/v1/chat/completions";

fn testRoute(model: []const u8) Route {
    if (std.mem.startsWith(u8, model, "go/")) {
        return .{ .endpoint = test_endpoint_go, .wire_model = model[3..] };
    }
    return .{ .endpoint = test_endpoint_zen, .wire_model = model };
}
...
const test_spec = Spec{
    ...
    .resolve_route = testRoute,
    .non_retryable_limit_markers = &.{ "GoUsageLimitError", "FreeUsageLimitError" },
};

test "route resolvers can select endpoints and wire model identities" {
    const zen = testRoute("kimi-k3");
    try std.testing.expectEqualStrings(test_endpoint_zen, zen.endpoint);
    try std.testing.expectEqualStrings("kimi-k3", zen.wire_model);
    const go = testRoute("go/kimi-k3");
    try std.testing.expectEqualStrings(test_endpoint_go, go.endpoint);
    try std.testing.expectEqualStrings("kimi-k3", go.wire_model);
    try std.testing.expectEqualStrings("", testRoute("go/").wire_model);
}

test "OpenCode usage-limit error types disable transient retries" {
    try std.testing.expect(isNonRetryableLimitError(
    ...
    try std.testing.expectEqual(
        ...
        failureKindWithDetail(.too_many_requests, "{\"error\":{\"type\":\"GoUsageLimitError\"}}", &test_spec),
    );
    try std.testing.expectEqual(
        stream_provider.FailureKind.rate_limited,
        failureKindWithDetail(.too_many_requests, "{\"error\":{\"type\":\"RateLimitError\"}}", &test_spec),
    );
}
...
test "buildRequest rejects empty wire model and oversized model ids" {
    ...
    try std.testing.expectError(error.InvalidOpenAICompatibleModel, buildRequest(std.testing.allocator, base, &test_spec));
    var oversized: [257]u8 = undefined;
    @memset(&oversized, 'a');
    try std.testing.expectError(error.InvalidOpenAICompatibleModel, buildRequest(std.testing.allocator, .{
        .model = &oversized,
        .messages = &messages,
        .tool_choice = .none,
        .provider_options = .{},
...
test "OpenCode SSE requires both finish reason and done sentinel" {
    ...
    try std.testing.expectError(error.OpenAICompatibleStreamIncomplete, consumeSse(
        std.testing.allocator,
        &reader,
        &callback_ctx,
        ignoreTestChunk,
