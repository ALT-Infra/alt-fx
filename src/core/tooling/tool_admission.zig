...
fn requestPermissionOutcomeResolved(
    ...
    var project_admission = if (is_mcp_tool)
        try tool_mcp_runtime.projectAdmissionForTool(
            input.mcp_runtime,
            arena,
            call.name,
        )
        ...
        null;
    defer if (project_admission) |*value| value.deinit(arena);
    if (project_admission) |project| {
        if (project.interactive_trust) switch (project.admission) {
            .approved => {},
            .rejected => return .{
                .decision = .policy_denied,
                .denial_reason = .policy_denied,
            },
            .pending => return requestProjectMcpPermissionOutcome(
                input,
                arena,
                call,
                project,
...
fn requestProjectMcpPermissionOutcome(
    input: Input,
    arena: Allocator,
    call: ToolCall,
    project: tool_mcp_runtime.ProjectToolAdmission,
) !command_admission.PermissionOutcome {
    const permission_name = try permissionNameForCall(input, arena, call);
    const target_kind = try permissionTargetKindForCall(input, arena, call);
    ...
    defer targets.deinit(arena);
    for (targets.items) |target| {
        if (try permissions.ruleDecisionFor(
            arena,
            input.permission_rules,
    ...
    if (input.session_permission_state) |state| {
        if (permissionStateKeyForCall(input, arena, call) catch null) |key| {
            if (session_permission_state.decide(state.*, key) == .deny) {
                return .{
                    .decision = .policy_denied,
                    .denial_reason = .policy_denied,
                };
    ...
    const prompter = input.permission_prompter orelse return .{
        .decision = .permission_required,
        .denial_reason = .permission_required,
        .requirement = .approval_required,
    };
    const request = try projectMcpPermissionRequest(
        input,
        arena,
        call,
        project.server_name,
    );
    const grant_offer = try exactApprovalLocalGrants(
        input,
        arena,
        call,
        &.{},
    ...
    var response = prompter.request(
        ...
        error.PermissionPromptUnavailable => return .{
            .decision = .permission_required,
            .denial_reason = .permission_required,
            .requirement = .approval_required,
        },
    ...
    if (response.decision == .always) {
        var refreshed = try tool_mcp_runtime.projectAdmissionForTool(
            input.mcp_runtime,
            arena,
            call.name,
        ) orelse return .{
            .tool_failure = "Project MCP approval changed the runtime, but the tool is no longer available.",
        };
        defer refreshed.deinit(arena);
        if (refreshed.admission != .approved or !std.mem.eql(
            u8,
            refreshed.server_name,
            project.server_name,
        )) return .{
            .tool_failure = "Project MCP approval did not publish the expected server authority.",
        };
        outcome.project_mcp_retry = .{
            .server_name = try arena.dupe(u8, refreshed.server_name),
...
fn projectMcpPermissionRequest(
    input: Input,
    arena: Allocator,
    call: ToolCall,
    ...
    const explanation = try std.fmt.allocPrint(
        arena,
        "Project MCP server '{s}' is defined in .mcp.json in this workspace.",
        .{encoded.bytes},
    );
    const arguments = try text_utils.encodeTerminalSafe(
        arena,
        call.arguments_json,
        permission_request.max_tool_arguments_preview_bytes,
    );
    return .{
        .label = try tool_presentation.formatPermissionLabel(
            arena,
            input.tool_registry,
            call,
        ),
        .explanation = explanation,
        .tool_arguments_preview = arguments.bytes,
        .project_mcp_server = server_name,
    };
}
...
test "pending project MCP tool uses one human prompt and returns typed retry on always" {
    const Fixture = struct {
        approved: bool = false,
        prompts: usize = 0,

        fn hasTool(_: *anyopaque, name: []const u8, _: tool_mcp_runtime.Access) bool {
            return std.mem.eql(u8, name, "mcp_docs_query");
        }
        ...
        fn admission(
            ...
            if (!std.mem.eql(u8, name, "mcp_docs_query")) return null;
            const self: *@This() = @ptrCast(@alignCast(raw));
            return .{
                .server_name = try alloc.dupe(u8, "docs"),
                .admission = if (self.approved) .approved else .pending,
                .interactive_trust = true,
                .runtime_generation = if (self.approved) 2 else 1,
            };
        }
        ...
        fn prompt(
            ...
            const self: *@This() = @ptrCast(@alignCast(raw));
            self.prompts += 1;
            try std.testing.expectEqualStrings("docs", request.project_mcp_server.?);
            try std.testing.expect(std.mem.find(u8, request.explanation.?, ".mcp.json") != null);
            self.approved = true;
    ...
    defer background.deinit(std.testing.allocator);
    var fixture = Fixture{};
    const advertised = [_][]const u8{"mcp_docs_query"};
    var input = testInputWithClassifier(
        &worker,
        &background,
        permission_auto_classifier.Classifier.disabled(),
    ...
    input.advertised_dynamic_tool_names = &advertised;
    input.permission_prompter = .{
        .context = &fixture,
        .request_fn = Fixture.prompt,
    };
    input.mcp_runtime = .{
        .context = &fixture,
        .has_tool = Fixture.hasTool,
        .project_tool_admission = Fixture.admission,
    };
    ...
    const outcome = try requestPermissionOutcome(
        ...
        .{
            .id = "project-mcp",
            .name = "mcp_docs_query",
            .arguments_json = "{\"q\":\"zig\"}",
        },
        .yolo,
        &.{},
    );
    try std.testing.expectEqual(@as(usize, 1), fixture.prompts);
    try std.testing.expectEqual(ToolPermissionDecision.always, outcome.decision);
    try std.testing.expectEqualStrings("docs", outcome.project_mcp_retry.?.server_name);
    try std.testing.expectEqual(@as(u64, 2), outcome.project_mcp_retry.?.runtime_generation);
}
