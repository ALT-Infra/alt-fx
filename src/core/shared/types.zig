...
pub const GatewayConnectionStatusTone = enum {
    neutral,
    warning,
    success,
};

pub const GatewayConnectionStatus = union(enum) {
    ...
    pub fn tone(self: GatewayConnectionStatus) ?GatewayConnectionStatusTone {
        return switch (self) {
            .connecting => .neutral,
            .retrying => .warning,
            .recovered => .success,
    ...
    pub fn isRecovered(self: GatewayConnectionStatus) bool {
        return switch (self) {
            .recovered => true,
            .connecting, .retrying, .clear => false,
        };
    }

    pub fn label(self: GatewayConnectionStatus, buf: []u8) ?[]const u8 {
        return switch (self) {
            .connecting => "Connecting...",
            .retrying => |attempt| std.fmt.bufPrint(
                buf,
                "Connection failed · attempt {d}/{d} · retrying",
                .{ attempt.number, attempt.limit },
            ) catch "Connection failed · retrying",
            .recovered => |attempt| std.fmt.bufPrint(
                buf,
                "Connection recovered · attempt {d}/{d}",
                .{ attempt.number, attempt.limit },
            ) catch "Connection recovered",
            .clear => null,
        };
    }
};

test "gateway connection status projects shared labels and tones" {
    var buf: [128]u8 = undefined;

    const connecting: GatewayConnectionStatus = .connecting;
    try std.testing.expectEqual(GatewayConnectionStatusTone.neutral, connecting.tone().?);
    try std.testing.expectEqualStrings("Connecting...", connecting.label(&buf).?);
    try std.testing.expect(!connecting.isRecovered());

    const retrying: GatewayConnectionStatus = .{ .retrying = .{ .number = 1, .limit = 3 } };
    try std.testing.expectEqual(GatewayConnectionStatusTone.warning, retrying.tone().?);
    try std.testing.expectEqualStrings(
        "Connection failed · attempt 1/3 · retrying",
        retrying.label(&buf).?,
    );

    const recovered: GatewayConnectionStatus = .{ .recovered = .{ .number = 2, .limit = 3 } };
    try std.testing.expectEqual(GatewayConnectionStatusTone.success, recovered.tone().?);
    try std.testing.expectEqualStrings(
        "Connection recovered · attempt 2/3",
        recovered.label(&buf).?,
    );
    try std.testing.expect(recovered.isRecovered());

    const clear: GatewayConnectionStatus = .clear;
    try std.testing.expect(clear.tone() == null);
    try std.testing.expect(clear.label(&buf) == null);
}

test "gateway connection status label fallbacks stay bounded" {
    var tiny_buf: [1]u8 = undefined;
    const retrying: GatewayConnectionStatus = .{ .retrying = .{ .number = 0, .limit = 0 } };
    const recovered: GatewayConnectionStatus = .{ .recovered = .{ .number = 0, .limit = 0 } };

    try std.testing.expectEqualStrings("Connection failed · retrying", retrying.label(&tiny_buf).?);
    try std.testing.expectEqualStrings("Connection recovered", recovered.label(&tiny_buf).?);
}
