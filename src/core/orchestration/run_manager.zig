...
fn contextSurfaceFixturePrompt(alloc: Allocator, text: []const u8) !worker_runtime.QueuedPrompt {
    return .{
        .prompt = try alloc.dupe(u8, text),
        .images = try alloc.alloc(types.ImageAttachment, 0),
        .authorized_image_catalog = try alloc.alloc(types.ImageAttachment, 0),
        .model = try alloc.dupe(u8, "go/kimi-k3"),
        .provider = .opencode,
        .api_key = try alloc.dupe(u8, "fixture-secret"),
...
test "context-bearing surface restores exact history without repeating the same root turn" {
    const alloc = std.testing.allocator;
    var manager = Manager{ .alloc = alloc };
    defer manager.deinit();

    const history = try alloc.alloc(types.HistoryTurn, 1);
    history[0] = try session_runtime.makeAssistantTurn(
        alloc,
        "canonical root sentinel",
        "first run tool trajectory sentinel",
    );
    try manager.commitContextSurface("coder", 17, &.{}, history);

    var continuation = try contextSurfaceFixturePrompt(alloc, "canonical root sentinel");
    defer worker_runtime.freeQueuedPrompt(alloc, continuation);
    try std.testing.expect(try manager.attachContextSurface(
        alloc,
        "coder",
        17,
        &.{},
        "new runtime snapshot sentinel",
        &continuation,
    ));
    try std.testing.expectEqualStrings("new runtime snapshot sentinel", continuation.prompt);
    try std.testing.expectEqual(@as(usize, 1), continuation.history.len);
    try std.testing.expect(std.mem.find(
        u8,
        continuation.history[0].assistant.user.text,
        "first run tool trajectory sentinel",
    ) != null);

    var next_turn = try contextSurfaceFixturePrompt(alloc, "next exact user turn sentinel");
    defer worker_runtime.freeQueuedPrompt(alloc, next_turn);
    try std.testing.expect(!try manager.attachContextSurface(
        alloc,
        "coder",
        18,
        &.{},
        "next runtime snapshot sentinel",
        &next_turn,
    ));
    try std.testing.expectEqualStrings("next exact user turn sentinel", next_turn.prompt);
    try std.testing.expectEqual(@as(usize, 1), next_turn.history.len);
}
