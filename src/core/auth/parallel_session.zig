...
const mutation_lock_file_name = "parallel-auth.lock";
const mutation_lock_deadline_ms: u64 = 2000;
const max_api_key_bytes: usize = 8 * 1024;
const auth_file_name = profile_paths.parallel_auth_file_name;
...
pub fn loadConfigured(alloc: Allocator) !?Session {
...
fn loadFromDir(alloc: Allocator, fx_dir: *std.Io.Dir) !?Session {
    var file = fx_dir.openFile(io_mod.getIo(), auth_file_name, .{
        ...
        error.FileNotFound => return null,
            ...
            debug_trace.logf("auth", "Parallel connection load failed step=open_file err={s}", .{@errorName(err)});
            return null;
        },
    };
    ...
    if (stat.kind != .file or stat.permissions.toMode() & 0o077 != 0) {
        debug_trace.logf("auth", "Parallel connection load failed step=permissions err=InsecureAuthFile", .{});
        return null;
    }
    ...
    return parse(alloc, bytes) catch |err| switch (err) {
        error.OutOfMemory => return err,
            ...
            debug_trace.logf("auth", "Parallel connection load failed step=parse err={s}", .{@errorName(err)});
            return null;
        },
    };
}
...
pub fn saveNewSession(alloc: Allocator, session: Session) !void {
    if (comptime host_target.is_wasm) return error.ParallelAuthUnavailable;
...
fn parse(alloc: Allocator, bytes: []const u8) !Session {
    if (bytes.len > max_auth_file_bytes) return error.ParallelAuthFileTooLarge;
    var parsed = try std.json.parseFromSlice(std.json.Value, alloc, bytes, .{});
    defer parsed.deinit();
    if (parsed.value != .object) return error.InvalidParallelAuthFile;
    const version = parsed.value.object.get("schema_version") orelse return error.InvalidParallelAuthFile;
    if (version != .integer or version.integer != schema_version) return error.InvalidParallelAuthFile;
    const api_key_value = parsed.value.object.get("api_key") orelse return error.InvalidParallelAuthFile;
    if (api_key_value != .string or !validApiKey(api_key_value.string)) return error.InvalidParallelAuthFile;
    return .{ .api_key = try alloc.dupe(u8, api_key_value.string) };
}

fn stringify(alloc: Allocator, session: Session) ![]u8 {
    if (!validApiKey(session.api_key)) return error.InvalidParallelAuthFile;
    var out: std.Io.Writer.Allocating = .init(alloc);
    errdefer out.deinit();
    try out.writer.writeAll("{\"schema_version\":1,\"api_key\":");
...
test "Parallel connection round trips and rejects unsafe secrets" {
    const alloc = std.testing.allocator;
    var session = Session{ .api_key = try alloc.dupe(u8, "parallel_test_key") };
    ...
    try std.testing.expectError(error.InvalidParallelAuthFile, parse(alloc, "{\"schema_version\":2,\"api_key\":\"x\"}"));
}
