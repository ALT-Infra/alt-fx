...
pub const StoreSpec = struct {
    auth_file_name: []const u8,
    mutation_lock_file_name: []const u8,
...
const Mutation = struct {
    ...
    fn save(self: *Mutation, alloc: Allocator, session: Session) !void {
        ...
        try io_mod.durableReplaceVerified(alloc, &self.fx_dir, self.spec.auth_file_name, text);
    }

    fn delete(self: *Mutation) !DeleteOutcome {
        self.fx_dir.dir.deleteFile(io_mod.getIo(), self.spec.auth_file_name) catch |err| switch (err) {
...
pub fn load(alloc: Allocator, spec: StoreSpec) !?Session {
    ...
    return loadFromDir(alloc, &fx_dir, spec);
}

fn loadFromDir(alloc: Allocator, fx_dir: *std.Io.Dir, spec: StoreSpec) !?Session {
    var file = fx_dir.openFile(io_mod.getIo(), spec.auth_file_name, .{
        .mode = .read_only,
        ...
        error.FileNotFound => return null,
            ...
            debug_trace.logf("auth", "{s} API-key session load failed step=open_file err={s}", .{ spec.provider_name, @errorName(err) });
            return null;
        },
    };
    ...
    if (stat.kind != .file or stat.permissions.toMode() & 0o077 != 0) {
        debug_trace.logf("auth", "{s} API-key session load failed step=permissions err=InsecureAuthFile", .{spec.provider_name});
        return null;
    }
    ...
    return parse(alloc, bytes) catch |err| switch (err) {
        error.OutOfMemory => return err,
            ...
            debug_trace.logf("auth", "{s} API-key session load failed step=parse err={s}", .{ spec.provider_name, @errorName(err) });
            return null;
        },
    };
}

pub fn saveNewSession(alloc: Allocator, session: Session, spec: StoreSpec) !void {
    if (comptime host_target.is_wasm) return error.ApiKeyAuthUnavailable;
    var mutation = try beginMutation(spec);
    defer mutation.deinit();
    try mutation.save(alloc, session);
}

pub fn logout(spec: StoreSpec) !DeleteOutcome {
    var mutation = (try beginExistingMutation(spec)) orelse return .missing;
    defer mutation.deinit();
    return mutation.delete();
}

fn beginExistingMutation(spec: StoreSpec) !?Mutation {
    if (comptime host_target.is_wasm) return null;
    ...
    return try lockMutation(fx_dir, spec);
}

fn beginMutation(spec: StoreSpec) !Mutation {
    const home = io_mod.getenv("HOME") orelse return error.HomeNotSet;
    var home_dir = io_mod.VerifiedDir{
        .dir = try std.Io.Dir.openDirAbsolute(io_mod.getIo(), home, .{ .iterate = true }),
    };
    ...
    return lockMutation(fx_dir, spec);
}

fn lockMutation(open_fx_dir: io_mod.VerifiedDir, spec: StoreSpec) !Mutation {
    var fx_dir = open_fx_dir;
    errdefer fx_dir.close();
    var lock = try io_mod.acquireTimedAdvisoryLock(
        &fx_dir,
        spec.mutation_lock_file_name,
        mutation_lock_deadline_ms,
    );
    errdefer lock.release();
    return .{ .fx_dir = fx_dir, .lock = lock, .spec = spec };
}
...
fn parse(alloc: Allocator, bytes: []const u8) !Session {
    if (bytes.len > max_auth_file_bytes) return error.ApiKeyAuthFileTooLarge;
    var parsed = try std.json.parseFromSlice(std.json.Value, alloc, bytes, .{});
    defer parsed.deinit();
    if (parsed.value != .object) return error.InvalidApiKeyAuthFile;
    const version = parsed.value.object.get("schema_version") orelse return error.InvalidApiKeyAuthFile;
    if (version != .integer or version.integer != schema_version) return error.InvalidApiKeyAuthFile;
    const api_key_value = parsed.value.object.get("api_key") orelse return error.InvalidApiKeyAuthFile;
    if (api_key_value != .string) return error.InvalidApiKeyAuthFile;
    if (!validApiKey(api_key_value.string)) return error.InvalidApiKeyAuthFile;
    return .{ .api_key = try alloc.dupe(u8, api_key_value.string) };
}

fn stringify(alloc: Allocator, session: Session) ![]u8 {
    if (!validApiKey(session.api_key)) return error.InvalidApiKeyAuthFile;
...
const test_spec = StoreSpec{
    .auth_file_name = "test-auth.json",
    .mutation_lock_file_name = "test-auth.lock",
    .provider_name = "Test",
};

test "API-key auth session round trips and validates its schema" {
...
test "API keys are bounded before serialization" {
...
test "store specs keep provider state paths distinct" {
    try std.testing.expect(!std.mem.eql(u8, test_spec.auth_file_name, "other-auth.json"));
    try std.testing.expectEqualStrings("Test", test_spec.provider_name);
}
