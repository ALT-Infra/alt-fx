...
test "history projection keeps system role only for leading summaries" {
    ...
    try vercel_protocol.validateToolMessageHistory(alloc, chat_messages.items);
    ...
    try std.testing.expect(saw_nonleading_summary);
    try std.testing.expect(saw_interruption_marker);
    try vercel_protocol.validateToolMessageHistory(alloc, budgeted_chat_messages.items);
}
...
test "compacted Unicode history serializes system content as a string" {
    ...
    const body = try vercel_protocol.buildGatewayRequestBody(arena, "[]", messages.items);
    const shape = try vercel_protocol.formatGatewayRequestShapeSummary(arena, body);
