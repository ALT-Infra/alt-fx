...
test "gateway connection status projects lifecycle and recovered survives turn summary until expiry" {
    const alloc = std.testing.allocator;
    var runtime = TranscriptRuntime{
        .layout = .{
            .rows = 24,
    ...
    try std.testing.expect(runtime.worker_status.set_gateway_connection(.connecting, 1_000));
    switch (runtime.activityProjection()) {
        .turn_thinking => |thinking| {
            try std.testing.expectEqual(activity_runtime.ActivityProjection.Tone.neutral, thinking.tone);
            try std.testing.expectEqualStrings("Connecting...", thinking.label);
        },
        .none, .tool_slot => return error.TestUnexpectedResult,
    }

    try std.testing.expect(runtime.worker_status.set_gateway_connection(.{
        .retrying = .{ .number = 1, .limit = 3 },
    }, 1_000));
    switch (runtime.activityProjection()) {
        .turn_thinking => |thinking| {
            try std.testing.expectEqual(activity_runtime.ActivityProjection.Tone.warning, thinking.tone);
            try std.testing.expectEqualStrings("Connection failed · attempt 1/3 · retrying", thinking.label);
        },
        .none, .tool_slot => return error.TestUnexpectedResult,
    }
