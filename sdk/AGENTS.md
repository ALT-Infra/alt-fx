...
- The WebAssembly runtime is not the native runtime. Keep native tools disabled. The optional workspace host may expose only the foreground `run_command` tool through its typed boundary and permission policy. Any additional capability requires its own typed host boundary, permission review where applicable, and coverage on the affected surface.
